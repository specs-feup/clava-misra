...
describe("Expressions", () => {
    ...
    it("should pass", () => {
        expectNumberOfErrors(reporter, pass, 4, Query.search(FileJp, {name: "good.c"}).first() as Joinpoint);
    });
    
    it("should fail", () => {
        expectNumberOfErrors(reporter, pass, 11, Query.search(FileJp, {name: "bad.c"}).first() as Joinpoint);
    });
});