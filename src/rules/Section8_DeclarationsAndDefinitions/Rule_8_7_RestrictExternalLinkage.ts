...
export default class Rule_8_7_RestrictExternalLinkage extends MISRARule {
    ...
    match($jp: Joinpoint, logErrors: boolean = false): boolean {
        ...
        if (nonCompliant && logErrors) {
            this.logMISRAError(
                $jp, 
                `${$jp instanceof FunctionJp ? "Function" : "Object"} '${$jp.name}' has external linkage but is only referenced within a single translation unit. Consider using the 'static' keyword to give it internal linkage.`
            );
        } 
        return nonCompliant;
    }
